...
from networks import GFlowNet, LatentDictionary, Decoder, PixelCNN
...
def sample_prior(prior, batch_size=128):
    ...
    for i in range(lh*lw):
        ...
        prior_pred_val = torch.multinomial(prior_pred.view(batch_size, dict_size, lh*lw)[:,:,i], 1)
...
def sleep_step(gfn, decoder, latent_dict, prior, batch_size=128):
    ...
    random_latent_state = sample_prior(prior, batch_size=batch_size).view(batch_size, dict_size*lh*lw)
    random_latent = random_latent_state.view(batch_size, dict_size, lh, lw).argmax(1).view(batch_size, lh*lw)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--reduce_lr_on', type=int, default=300, help='Epoch to reduce lr on')
    ...
    prior.train()
    dec_opt = torch.optim.Adam(list(decoder.parameters()) + list(latent_dict.parameters()) + list(prior.parameters()), lr=args.dec_lr)
        ...
        for batch in batch_bar:
            ...
            if stage == 'gfn':
                ...
                reward = (-F.binary_cross_entropy(pred_img, img, reduction='none').sum((1,2,3)) / steps 
                          -F.cross_entropy(prior_pred, state.view(batch_size, gfn.dictionary_size, gfn.lh, gfn.lw), reduction='none').sum((1,2)) / steps)
                ...
                if np.random.rand() < args.sleep_prob:
                    ...
                    logprobs, _ = sleep_step(gfn, decoder, latent_dict, prior, batch_size=args.batch_size)
            ...
            # M-step: Train decoder using sampled latents
                ...
                state, logprobs = sample(gfn, img, p=1, rand_prob=0)    
                prior_pred = prior(state.view(batch_size, gfn.dictionary_size, gfn.lh, gfn.lw))          
                prior_loss = F.cross_entropy(prior_pred, state.view(batch_size, gfn.dictionary_size, gfn.lh, gfn.lw))
    ...
    for batch in tqdm.tqdm(test_dataloader):
        ...
        for b in range(img.shape[0]):
            ...
            with torch.no_grad():
                ...
                logpz = -F.cross_entropy(prior_pred, state.view(x.shape[0], gfn.dictionary_size, gfn.lh, gfn.lw), reduction='none').sum((1,2))
